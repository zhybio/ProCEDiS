...
def worker(
    ...
    try:
        ...
        print(f"[Worker {local_device_id}] Error traceback:", file=sys.stderr)
...
def parse_args():
    p = argparse.ArgumentParser(description="Step 02_2: OpenFold inference for RL-selected A3Ms.")
    ...
    p.add_argument("--names", nargs="*", default=None, help="Run only these targets.")
    ...
    p.add_argument(
        "--skip_existing",
        action="store_true",
        help="Skip A3Ms whose output PDB already exists in <results_dir>/<name>/output/.",
    )
    return p.parse_args()
...
def main():
    ...
    for name in names:
        t0 = time.time()
        print(f"[02_2] {name} started")

        fasta_path = resolve_fasta(args.inputs_dir, name)
        with open(fasta_path, "r") as f:
            sequence = f.readlines()[1].strip()

        a3m_list = glob.glob(os.path.join(args.results_dir, name, "result", "*.a3m"))
        a3m_list.sort()

        output_dir = os.path.join(args.results_dir, name, "output")
        os.makedirs(output_dir, exist_ok=True)

        if not a3m_list:
            print(f"[WARN] No A3M files found for {name} under {args.results_dir}/{name}/result/")
            continue
        ...
        if args.skip_existing:
            ...
            a3m_list = kept
            if skipped > 0:
                print(f"[02_2] {name} skip_existing: skipped {skipped} already-done PDBs")

        if not a3m_list:
            print(f"[02_2] {name} nothing to do")
            continue
        ...
        print(f"[02_2] {name} done in {(time.time() - t0):.3f}s")
