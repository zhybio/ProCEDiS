...
def _placeholder_template_feats(num_templates: int, num_res: int):
    return {
...
class OpenfoldPredictor:
    ...
    def __init__(
        ...
        preset = f"model_{model_idx}"  # Non-PTM by design
        self.config_preset = preset

        self.cfg = config.model_config(preset)
        openfold_model = model.AlphaFold(self.cfg).eval()

        params_file = pd_dir / f"params_{preset}.npz"
        if not params_file.exists():
            raise FileNotFoundError(f"Params file not found: {params_file}")

        self._import_jax_weights(openfold_model, str(params_file), version=preset)

        if precision is not None:
            openfold_model = openfold_model.to(precision)

        self.openfold_model = openfold_model.to(device)
        self.device = device
    ...
    def inference(
        ...
        # Update cfg in-place (fast). Each process has its own predictor instance.
        ...
        with torch.no_grad():
            pred = self.openfold_model(processed)

        processed_np = self._tensor_tree_map(lambda t: np.array(t[..., -1].cpu()), processed)
        pred_np = self._tensor_tree_map(lambda t: np.array(t.cpu()), pred)
    ...
    def rl_inference(self, a3m_text: str, sequence: str, max_msa_clusters: int = 256):
        # Fast RL inference: returns CA coords [N,3] and mean pLDDT (scalar).
        self.cfg.data.predict.max_extra_msa = 1
        self.cfg.data.predict.max_msa_clusters = max_msa_clusters
        self.cfg.data.common.max_recycling_iters = 0
        ...
        fa = pred["final_atom_positions"]
        if fa.dim() == 4:
            coords = fa[0, :, 1, :]   # [B,N,37,3] -> CA
        elif fa.dim() == 3:
            coords = fa[:, 1, :]      # [N,37,3] -> CA
        else:
            raise RuntimeError(f"Unexpected final_atom_positions shape: {tuple(fa.shape)}")

        plddt = pred["plddt"].mean()
        return coords.detach().cpu(), plddt.detach().cpu()
